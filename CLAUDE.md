...
- **The selection layer (取捨選択) is set in two places, not on 編集方針**
  ...
  **content filtering** (コンテンツフィルタリング) is on the 文書 screen
  above the list: the criteria for keeping / dropping a document, meant
  for an LLM that reads the fetched document, not built yet.
  `EditorialPolicy::LAYERS` has five layers.
