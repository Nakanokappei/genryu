...
<section class="w-full space-y-6" @if ($article->status === 'generating') wire:poll.5s="refreshStatus" @endif>
    <x-pages::detail-header :back="route('articles.index')" :back-label="__('Articles')" :title="$article->displayTitle()" />

    <x-pages::fields :fields="[
        __('Source') => $article->material?->document->updateEntry->source->name,
